...
const EditCliente = () => {
  ...
  useEffect(()=>{
    axios.get(`http://localhost:5555/clientes/${id}`).then((response)=>{
  ...
  const enviarSolicitud = async (parametros) => {
    await axios.put(`http://localhost:5555/clientes/${id}`, parametros).then( (respuesta) => {
      if (respuesta.status == 200) {
        alert("Registrado editado exitosamente")
      }
      navigate(-1)
    })
  }
